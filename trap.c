...
void kerneltrap(void) {
    // 保存中断发生时的CSR寄存器值
    // 保存现场
    unsigned long sepc = r_sepc();       // 异常程序计数器
    unsigned long sstatus = r_sstatus(); // 状态寄存器
    unsigned long scause = r_scause();   // 异常原因寄存器
    ...
    if ((scause >> 63) == 0) {
        ...
        // }
        struct trapframe tf;
        tf.sepc = sepc;    // 异常发生的PC
        tf.sstatus = sstatus;  // 状态寄存器
        tf.stval = r_stval();  // 异常相关值（如出错地址）
        tf.scause = scause;  // 异常原因
